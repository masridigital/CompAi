...
export class AdminUsersService {
  ...
  async setGlobalRole({
    ...
    const { updated, deactivatedMemberIds } = await db.$transaction(async (tx) => {
      const updatedUser = await tx.user.update({
        where: { id: userId },
        data: { role },
        select: { id: true, email: true, name: true, role: true },
      });
      if (!demotingStaff) return { updated: updatedUser, deactivatedMemberIds: [] };

      // Demoted techs lose their MSP access: deactivate memberships that carry
      // msp_tech. Client-owned memberships with other roles stay untouched.
      const memberships = await tx.member.findMany({
        where: { userId, deactivated: false },
        select: { id: true, role: true },
      });
      const ids = memberships
        .filter((m) => m.role.split(',').map((r) => r.trim()).includes(MSP_TECH_ROLE))
        .map((m) => m.id);
      if (ids.length > 0) {
      ...
      return { updated: updatedUser, deactivatedMemberIds: ids };
    });
    ...
    if (deactivatedMemberIds.length > 0) {
      this.logger.log(
        `Deactivated ${deactivatedMemberIds.length} msp_tech membership(s) of ${userId}: ${deactivatedMemberIds.join(', ')}`,
      );
    }
    return updated;
  }
